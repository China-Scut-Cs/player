...
//            dataSize = av_samples_get_buffer_size(NULL, player->apCodecCtx->channels,
//                        pFrame->nb_samples, player->apCodecCtx->sample_fmt, 1);
//            memcpy(player->audioBuffer, pFrame->data[0], dataSize);//将解码数据复制到输出缓存
            SDL_UnlockMutex(AudioPlay::qlock);
            if(dataSize<=0) {
                continue;
            }
...
//        memcpy(stream, (uint8_t *)player->audioBuffer + player->audioBufferIndex, length);
