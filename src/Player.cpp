...
//    if (pCodecParam->codec_type == AVMEDIA_TYPE_AUDIO) {     //检查解码器类型是否为音频解码器
//        //wanted_spec是进程请求SDL库的音频参数，spec是SDL库返回给进程它能支持的音频参数，如果请求超过SDL支持返回，则返回最相近参数
//        SDL_AudioSpec wanted_spec, spec;
//        // 创建SDL_AudioSpec结构体，设置音频播放参数
//        wanted_spec.freq = /*pCodecParam->sample_rate;*/ 44100;        //采样频率 DSP frequency -- samples per second
//        wanted_spec.format = AUDIO_S16SYS;              //采样格式
//        wanted_spec.channels = /*pCodecParam->channels;*/  av_get_channel_layout_nb_channels(AV_CH_LAYOUT_STEREO);    //声道数 Number of channels: 1 mono, 2 stereo
//        wanted_spec.silence = 0;                            //无输出时是否静音
//        //默认每次读音频缓存的大小，推荐值为 512~8192
//        wanted_spec.samples = SDL_AUDIO_BUFFER_SIZE;
//        wanted_spec.callback = AudioPlay::audio_callback;//设置读取音频数据的回调接口函数
//        wanted_spec.userdata = player;//传递用户数据
    ...
    switch (pCodecCtx->codec_type) {
        case AVMEDIA_TYPE_AUDIO://音频解码器
            ...
            player->swr_ctx = swr_alloc_set_opts(NULL,
                            /*av_get_default_channel_layout(pCodecParam->channels),*/
                            av_get_default_channel_layout(spec.channels),   //输出
                            AV_SAMPLE_FMT_S16,
                            /*pCodecParam->sample_rate,*/
                            spec.freq,                     //输出
                            av_get_default_channel_layout(pCodecCtx->channels),     //输入
...
//			player->audioBufferSize = 0;        //解码后的多帧音频数据长度
//			player->audioBufferIndex = 0;           //累计送入声卡的长度
